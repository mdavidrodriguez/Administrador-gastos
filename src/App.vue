...
onMounted(() => {
  ...
  if (datosGuardados) {
    gastos.value = JSON.parse(datosGuardados)
  }
})
...
const guardarLocalstorage = () => {
  const gastosStorage = localStorage.setItem('gastos', JSON.stringify(gastos.value))
}

const resetApp = () => {
  if (confirm('¿Deseas reiniciar presupuesto y gasto?')) {
    gastos.value = []
    presupuesto.value = 0
  }
}
  ...
  <div :class="{ fijar: modal.mostrar }">
    ...
    <main v-if="presupuesto > 0">
      ...
      <Modal v-if="modal.mostrar" @ocultar-modal="ocultarModal" :modal="modal" v-model:nombre="gasto.nombre"
        v-model:cantidad="gasto.cantidad" v-model:categoria="gasto.categoria" :id="gasto.id" @guardar-gasto="guardarGasto"
        :disponible="disponible" @eliminar-gasto="eliminarGasto" />
    </main>
  </div>
</template>
